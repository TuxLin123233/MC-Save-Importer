...
## 简介
本文件面向存档管理器的GUI界面系统，重点围绕App类的架构设计与实现细节展开，涵盖主窗口布局、四个核心功能按钮的实现逻辑、进度条与消息对话框系统、CustomTkinter框架的使用方式、现代化主题设计与响应式布局原理，并提供可直接定位到源码的路径示例，帮助开发者快速理解与扩展界面系统。
...
- Message类：模态消息对话框封装，提供信息提示与“是/否”确认两种常用交互。
...
## 架构总览
App类通过CustomTkinter构建主窗口，采用两行四列的按钮网格布局，辅以标题区与图标。四个核心功能按钮分别绑定导入存档、导出存档、存档列表、赞助一下等操作；其中“导入存档”流程最为复杂，涉及Minecraft路径检测、版本迁移选择、ZIP批量解压、进度反馈与覆盖确认等步骤；“赞助一下”提供二维码展示窗口；其他功能按钮当前处于占位状态，提示“功能开发中”。
...
- 按钮区：两行四列布局，每行独立容器，按钮具有统一尺寸、圆角边框、悬停颜色与复合布局（图标在上、文字在下）。
- 四个核心按钮：
...
- [src/gui.py:413-517](file://src/gui.py#L413-L517)
- [src/gui.py:549-596](file://src/gui.py#L549-L596)
...
图表来源
- [src/gui.py:413-517](file://src/gui.py#L413-L517)
- [src/gui.py:518-548](file://src/gui.py#L518-L548)
...
### 响应式布局与主题设计
- 响应式布局：按钮容器使用两行四列布局，行内使用填充与对齐控制，确保在固定窗口尺寸下保持视觉平衡。
...
章节来源
- [src/gui.py:14-30](file://src/gui.py#L14-L30)
- [src/gui.py:59-166](file://src/gui.py#L59-L166)
...
章节来源
- [src/gui.py:90-166](file://src/gui.py#L90-L166)
- [src/gui.py:622-732](file://src/gui.py#L622-L732)
...
```mermaid
graph LR
App["App类(src/gui.py)"] --> CTk["CustomTkinter"]
App --> PC["PathConfig(src/config.py)"]
App --> UT["工具函数(src/utils.py)"]
Msg["Message类(src/gui.py)"] --> CTk
Msg --> PC
UT --> PIL["Pillow"]
UT --> Z["zipfile/shutil"]
...
章节来源
- [src/gui.py:263-302](file://src/gui.py#L263-L302)
- [src/gui.py:549-596](file://src/gui.py#L549-L596)
- [src/utils.py:4-32](file://src/utils.py#L4-L32)
...
章节来源
- [src/config.py:17-21](file://src/config.py#L17-L21)
- [src/config.py:77-90](file://src/config.py#L77-L90)
- [src/utils.py:15-31](file://src/utils.py#L15-L31)
- [src/gui.py:549-596](file://src/gui.py#L549-L596)

## 结论
App类以清晰的职责划分与模块化设计实现了简洁而实用的GUI界面，结合CustomTkinter的现代化主题与响应式布局，提供了良好的用户体验。导入存档流程体现了完整的路径检测、版本迁移、批量处理与进度反馈机制；Message类与进度窗口共同构成了可靠的用户交互闭环。未来可在配置统一化、功能完善化与性能优化方面继续演进。
