...
## 更新摘要
**变更内容**
- GUI系统已重构为基于App类的新架构，包含五个核心功能按钮而非原来的四个
- 新增完整的捐赠窗口系统，支持微信和支付宝二维码展示
- 改进的进度跟踪系统，提供更详细的进度反馈和文件名显示
- 方法重新组织，新增多个私有辅助方法用于版本选择、二维码展示和进度窗口管理
- 新增"存档修复"功能按钮，预留扩展空间
- 导出存档、存档列表、关于软件功能目前处于占位状态
...
## 简介
本文件面向存档管理器的GUI界面系统，重点围绕重构后的App类架构设计与实现细节展开，涵盖主窗口布局、五个核心功能按钮的实现逻辑、改进的进度条与消息对话框系统、CustomTkinter框架的使用方式、现代化主题设计与响应式布局原理，并提供可直接定位到源码的路径示例，帮助开发者快速理解与扩展界面系统。
...
## 核心组件
- App类：主窗口控制器，负责窗口初始化、标题与按钮区域创建、五个功能按钮的事件绑定与业务流程调度、进度条与消息对话框的创建与管理。
...
## 架构总览
App类通过CustomTkinter构建主窗口，采用两行三列的按钮网格布局，辅以标题区与图标。五个核心功能按钮分别绑定导入存档、导出存档、存档列表、存档修复、赞助一下等操作；其中"导入存档"流程最为复杂，涉及Minecraft路径检测、版本迁移选择、ZIP批量解压、进度反馈与覆盖确认等步骤；"赞助一下"提供二维码展示窗口；其他功能按钮当前处于占位状态，提示"功能开发中"。
...
+create_buttons()
+import_save()
+_select_version_saves(minecraft_path)
+export_save()
+list_saves()
+donate()
+about()
+_show_donate_qr(platform, parent_win)
+_progress_window(text)
}
class Message {
+window
+result
...
**章节来源**
- [src/app.py:8-38](file://src/app.py#L8-L38)
- [src/app.py:40-169](file://src/app.py#L40-L169)
- [src/app.py:171-306](file://src/app.py#L171-L306)
- [src/app.py:307-330](file://src/app.py#L307-L330)
- [src/app.py:331-446](file://src/app.py#L331-L446)
- [src/app.py:566-645](file://src/app.py#L566-L645)
...
**章节来源**
- [src/app.py:14-38](file://src/app.py#L14-L38)
- [src/app.py:62-169](file://src/app.py#L62-L169)
...
**章节来源**
- [src/app.py:93-169](file://src/app.py#L93-L169)
- [src/message.py:29-114](file://src/message.py#L29-L114)
...
- [src/app.py:597-645](file://src/app.py#L597-L645)
- [src/utils.py:4-32](file://src/utils.py#L4-L32)
