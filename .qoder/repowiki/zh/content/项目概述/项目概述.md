...
- [使用指南.md](file://wiki/使用指南.md)
- [开发文档.md](file://wiki/开发文档.md)
...
## 更新摘要
**变更内容**
- 新增完整的项目概览文档结构，包含 Home.md 提供的项目介绍、技术栈和使用说明
- 更新项目结构描述，反映实际的 src/app.py 替代原 gui.py 的架构变化
- 补充详细的开发文档和使用指南内容
- 更新核心组件分析，基于实际的 app.py 实现
- 增强技术栈和依赖关系分析
...
## 简介
本项目是一个面向 Minecraft Java 版的图形化存档管理工具，旨在简化存档导入、导出、备份与列表管理等常见操作。通过直观的图形界面，用户可以一键将 ZIP 地图解压到 .minecraft/saves 或版本迁移结构下的对应目录，并提供进度反馈与交互式确认机制。项目采用 Python 与 CustomTkinter 构建跨平台桌面应用，支持打包为独立可执行文件，便于非技术用户直接使用。
...
## 项目结构
项目采用"模块化分层"的组织方式，核心源代码位于 src 目录，资源文件（图片、字体、声音）分别置于 img、fonts、sounds 等目录，配置数据存储在 data.json 中。构建与打包脚本通过 requirements.txt 管理依赖，wiki 目录提供完整的项目文档。
...
**章节来源**
- [wiki/Home.md:12-22](file://wiki/Home.md#L12-L22)
...
## 核心组件
- **应用入口与生命周期**
  - main.py 负责导入配置与 App 类，创建 App 实例并启动主循环。
- **图形界面与交互**
  - app.py 定义 App 类，负责窗口布局、按钮事件绑定、导入/导出/列表等功能入口，以及消息弹窗与进度窗口的封装。
- **工具函数库**
...
- **消息对话框系统**
  - message.py 提供统一的消息提示和用户交互对话框，支持信息提示和确认对话框。
- **配置与状态**
...
```mermaid
graph TB
MAIN["main.py<br/>应用入口"] --> APP["App 类<br/>app.py"]
APP --> UTILS["工具函数库<br/>utils.py"]
APP --> CONFIG["路径配置<br/>config.py"]
APP --> MESSAGE["消息系统<br/>message.py"]
UTILS --> CONFIG
MESSAGE --> CONFIG
...
### 应用入口与生命周期
- **职责**
  - 导入配置与 App 类，创建 App 实例并启动 Tk 主循环。
- **设计要点**
  - 保持入口极简，逻辑集中在 App 类中，便于扩展与测试。
- **性能与稳定性**
...
- **模块间依赖**
  - main.py 依赖 config 与 app
  - app.py 依赖 config、utils、message
  - utils.py 依赖 config
  - message.py 依赖 config、utils
...
- **导出/列表功能提示开发中**
  - 当前版本尚未实现，后续版本将逐步完善。
- **版本迁移结构识别问题**
  - 确认 .minecraft 目录包含 versions 子目录，程序会自动识别版本迁移结构。

**章节来源**
- [src/utils.py:161-186](file://src/utils.py#L161-L186)
- [src/utils.py:4-32](file://src/utils.py#L4-L32)
- [src/app.py:307-329](file://src/app.py#L307-L329)

## 结论
本项目以简洁实用为目标，围绕 Minecraft Java 版存档导入这一核心需求，提供了直观易用的图形界面与稳健的底层实现。通过模块化设计与资源路径抽象，既满足初学者的使用需求，也为后续扩展（如导出、列表、修复等）预留了清晰接口。项目目前实现了核心的导入功能，其他功能（导出、列表、修复）正在开发中。建议在后续版本中完善剩余功能，并引入更完善的日志与错误报告机制，进一步提升可用性与可维护性。

## 附录
- **使用方法**
  - 打开程序，点击对应功能按钮，选择文件夹，等待完成。
  - 首次使用时，程序会自动检测 .minecraft 路径并提供版本选择。
- **开发与打包**
  - 安装依赖后，可在 src 目录使用 PyInstaller 打包为独立可执行文件，支持添加资源与 UPX 压缩。
- **项目文档**
  - wiki 目录包含完整的项目文档，包括使用指南、开发规范和常见问题解答。

**章节来源**
- [README.md:18-24](file://README.md#L18-L24)
- [README.md:42-86](file://README.md#L42-L86)
- [wiki/使用指南.md:18-26](file://wiki/使用指南.md#L18-L26)
- [wiki/开发文档.md:126-149](file://wiki/开发文档.md#L126-L149)